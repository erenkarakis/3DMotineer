    ...
    def spotLandmarks(self, img, draw=True):
        ...
        if self.results.pose_landmarks:
            for id, lms in enumerate(self.results.pose_landmarks.landmark):
                ...
                h, w, c = img.shape
                cx, cy, cz = int(lms.x*w), int(lms.y*h), int(lms.z*(self.zAxisMultiplier))
                #print(id, cx, cy, cz)
...
def main():
    cap = cv2.VideoCapture("Test/test11.mp4")
    cTime = 0
    pTime = 0
    ...
    while True:
        ...
        if len(lmlist) != 0:
            print(lmlist[16])
            cv2.circle(img, (lmlist[16][1], lmlist[16][2]), 10, (255, 255, 0), cv2.FILLED)
